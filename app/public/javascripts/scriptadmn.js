...
async function openUserDetail(id){
    if(!document.getElementById('userDetailId')){
        ...
        let result = await response.json();
        let html = `<div id="userDetatilIdHeader" class="userDetailHeader"></div><div class="userDetailInfo"><div class="column"><label for="username">Имя пользователя</label><br><label for="office">Офис</label><br><label for="role">Должность</label><br><label for="setpswrd">Сменить пароль</label></div><div class="column"><input type="Text" name="username" id="username" class="noselectable" value="${result[0].username}" readonly="true"><br><input type="Text" name="office" id="office" value="${result[0].office}"><br><select name="Role" id="selectId"><br><option value="1">Инженер</option><option value="2">Менеджер</option><option value="3">Администратор</option><input type="button" name="setpswrd" id="newpassId" onclick="setPass(${result[0].id})" value="Новый пароль"></div></div><div class="userDetailSubButtons"><input type="button" class="submitButton" onclick="saveUserInfo(${result[0].id})" value="Сохранить"><input type="button" class="submitButton" onclick="cancelForm('userDetailId')" value="Отмена"></div>`;
...
function newUser(){
       let html = `<div id="newUserFormIdHeader" class="newUserFormHeader"></div><div class="userDetailInfo"><div class="column"><label for="username">Имя пользователя</label><br><label for="password">Пароль</label><br><label for="role">Должность</label><br><label for="office">Офис</label><br></div><div class="column"><input type="text" id="username" name="username"><input type="text" id="password" name="password"><select id="role" name="role"><option value="user">Инженер</option><option value="moderator">Менеджер</option><option value="admin">Администратор</option><input type="text" id="office" name="office"></div></div><div class="userDetailSubButtons"><input type="button" onclick="saveNewUser()" value="Сохранить" class="submitButton"><input type="button" onclick="cancelForm('newUserFormId')" value="Отменить" class="submitButton"></div>`;
       let form = document.createElement('form');
